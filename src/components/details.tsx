...
const VideoDetails = async ({ type, id, currSeason, currEpisode }: { type: string, id: string, currSeason?: string, currEpisode?: string }) => {
    ...
    return (
        <div className='bg-gray-900 p-6 flex flex-col space-y-16 text-white'>
            <div className='flex space-x-4'>
                ...
                <iframe src={`https://vidsrc.to/embed/${type}/${id}${(type == "tv" ? `/${currSeason || 1}/${currEpisode || 1}` : "")}`} className='w-full h-full min-h-[80vh] rounded-lg' />
                </div>
            </div>
            ...
            <div className='flex space-x-6'>
                ...
                <div className="w-full text-white flex flex-col justify-between space-y-2">
                    <div className="px-4 text-xs font-bold bg-gray-900 rounded-lg flex justify-between w-full">
                        <div className="text-3xl font-bold bg-gray-900 rounded-lg">{title}</div>
                    ...
                    </div>                    <div className="min-h-8 px-4 text-sm">{tagline}</div>


                    {/* <div className="min-h-8 px-4 text-sm text-semibold">Genre :- {genres.map((genre:any) => genre.name).join(", ")}</div> */}
                    <div className="min-h-8 px-4 pb-2 text-sm text-gray-300 ">
                        {overview}
                    </div>
                    ...
                    <Property name='Release Date' value={moment(release_date).format("MMM D, YYYY")} />
                    <Property name='Rating' value={vote_average?.toString()?.slice(0, 3)} />
